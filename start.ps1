...
docker run --name deepdream-compute -v //c/Users/slant/clouddream/deepdream:/opt/deepdream -d docker.pkg.github.com/slanthie/clouddream/clouddream:0.0.1 bash -c "cd /opt/deepdream && ./process_images.sh 2>&1 > log.html"
docker run --name deepdream-json --volumes-from deepdream-compute -d ubuntu:14.04 bash -c "cd /opt/deepdream && ./make_json.sh"
